<%@ Page Title="" Language="C#" MasterPageFile="~/master_page/main.master" AutoEventWireup="true" CodeFile="ReportGroup.aspx.cs" Inherits="admin_ReportGroup" %>
                        ...
                        <div class="row">
									<div class="col-xs-12">
                                        ...
                                        <center>
                                            <table>
                                                <tr>
                                                    <td><%=Session["report_group_Column01"]%></td>
                                                    <td style="width:5px"></td>
                                                    <td>
                                                        <select class="chosen-select form-control" id="Type_Grp_ID" data-placeholder="<%=Session["search_placeholder"] %>" style="width:150px">
					                                        <option value=""></option>	
                                                    ...
                                                    <td style="width:5px"></td>
                                                    <td>
                                                        <select class="chosen-select form-control" id="Loc_ID" data-placeholder="<%=Session["search_placeholder"] %>"  style="width:150px">
					                                        <option value=""></option>	
                                            ...
                                            <div class="widget-body">
                                                <div class="widget-main no-padding">
											        ...
											        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
												        <thead>
													        <tr>
														        <th class="center" style="width:50px"><%=Session["report_group_ColumnSEQ"]%></th>
														        <th class="center"><%=Session["report_group_Column01"]%></th>
														        <th class="center"><%=Session["report_group_Column02"]%></th>
                                                                <th class="center" style="width:120px">
                                                                    <i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>
                                                                    <%=Session["report_group_Column04"]%>
                                                                </th>
                                                                <th class="center" style="width:100px">
                                                                    <i class="ace-icon fa fa-clock-o bigger-110 hidden-480"></i>
                                                                    <%=Session["report_group_Column05"]%>
                                                                </th>														      
														       <td class="center" style="width:80px"><%=Session["report_group_ColumnEdit"]%></td>
													        </tr>
												        </thead>
                                                        <tbody>
													        <tr ng-repeat="x in Data">
														        <td class="center">{{ x.RowID }}</td>
                                                                <td>{{ x.Type_Grp_Name }}</td>
														        <td>{{ x.Loc_Name }}</td>
...
<script>
    ...
    function fnMain($scope, $http) {
        ...
        $scope.fnNew = function () {
            fnOpenPopup('<%=Session["pop_add_report_group"]%>', "../admin_popup/pop_ReportGroup.aspx", null, "450");
        }
        $scope.fnEdit = function (KeyID) {
            fnOpenPopup('<%=Session["pop_edit_report_group"]%>', "../admin_popup/pop_ReportGroup.aspx?KeyID=" + KeyID, null, "450");
        }
        $scope.fnDelete = function (KeyID) {
            tmpKeyID = KeyID;
            fnConfirmMessage('<%=Session["pop_confirm_report_group"]%>', '<%=Session["pop_delete_report_group"]%>', fnDeleteYes);
        }
    ...
    function GetData($scope, $http, PageIndex) {
        ...
        .success(function (data, status, headers, config) {
            ...
            if (isLoad)
                setTimeout(fnGetReportGroupType, 100);
            else
                $('body').pleaseWait('stop');
        })
        .error(function (data, status, header, config) {
